...
* Fork the repo and create a new branch for the new release:
    ...
    ```bash
    gh repo fork fido-device-onboard/go-fdo-client --clone --remote
    git pull upstream main
    git checkout -b prepare-v0.0.1
    ```

* Update the `build/package/rpm/go-fdo-client.spec` file and set the new version: `Version: 0.0.1`
   ```bash
   sed -i 's/^Version:.*/Version: 0.0.1/' build/package/rpm/go-fdo-client.spec
   ```
...
* Commit all the changes and create a PR:
    ...
    git add build/package/rpm/go-fdo-client.spec go.mod go.sum
...
* Using the webui, open the [Releases](https://github.com/fido-device-onboard/go-fdo-client/releases)
