...
func onboardCmdInit() {
	...
	onboardCmd.Flags().StringVar(&onboardConfig.Onboard.Download, "download", "", "A dir to download files into (FSIM disabled if empty)")
	...
	onboardCmd.Flags().StringArrayVar(&onboardConfig.Onboard.Upload, "upload", nil, "List of dirs and files to upload files from, comma-separated and/or flag provided multiple times (FSIM disabled if empty)")
	onboardCmd.Flags().StringVar(&onboardConfig.Onboard.WgetDir, "wget-dir", "", "A dir to wget files into (FSIM disabled if empty)")
}
